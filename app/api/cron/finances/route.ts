 ...
 * 
 * Phase 3 of 4 - Runs at 6:10pm AEST (8:10 UTC)
...
import type { Player, Team, Notification } from '@/lib/game-engine/types';
import { 
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // ===========================================
    // SUNDAY: Reset weekly transfers + International arrivals
    // ===========================================
    ...
    if (isSunday) {
      ...
      // International free agent injection
      try {
    ...
    // Load players for roster counts
    const [p1, p2, p3] = await Promise.all([
      supabase.from('players').select('id, team_id').range(0, 999),
      supabase.from('players').select('id, team_id').range(1000, 1999),
      supabase.from('players').select('id, team_id').range(2000, 2999),
    ]);
